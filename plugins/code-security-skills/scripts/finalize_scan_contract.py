#!/usr/bin/env python3
# Modified from OpenAI Codex Security by the Code Security Skills maintainers; see ../PROVENANCE.md.
"""Validate, seal, and project standalone Code Security scan artifacts."""
...
SCHEMA_VERSION = "1.0"
PRODUCER_NAME = "code-security-skills"
FINGERPRINT_ALGORITHM = "code-security/v1"
...
SAFE_SCHEMA_PATTERNS = {
    r"^(?![^:/?#]+://[^/?#]*@)[^?#]+$",
    r"^code-security-snapshot/v1:sha256:[a-f0-9]{64}$",
    r"^(?!/)(?!.*(?:^|/)\.\.(?:/|$))(?!.*\\).+$",
    r"^[a-f0-9]{64}$",
    r"^(?!.*(?:^|/)\.\.(?:/|$))(?!.*\\)artifacts/.+$",
    r"^csf_[a-f0-9]{24}$",
    r"^occ_[a-f0-9]{24}$",
    r"^[a-z0-9][a-z0-9._/-]*$",
    r"^code-security/v1:sha256:[a-f0-9]{64}$",
...
class ContractError(ValueError):
    """Raised when a completed scan does not satisfy the standalone contract."""
...
def _generate_report_projection(
    ...
    spec = importlib.util.spec_from_file_location("code_security_report_projection", script)
...
def _windows_scan_local_files() -> Any:
    ...
    if _WINDOWS_SCAN_LOCAL_FILES is None:
        ...
        spec = importlib.util.spec_from_file_location("code_security_windows_scan_files", script)
        if spec is None or spec.loader is None:
...
def _populate_unsealed_manifest_envelope(
    ...
    """Populate non-semantic draft fields owned by standalone finalization."""

    manifest["documentType"] = "code-security.scan-manifest"
    manifest["schemaVersion"] = SCHEMA_VERSION
    scan["status"] = "completed"
    scan["coverageRef"] = "coverage.json"
    scan["findingsRef"] = "findings.json"
    started_at = os.environ.get("CODE_SECURITY_STARTED_AT")
    if started_at is not None:
        _validate_date_time(started_at, "CODE_SECURITY_STARTED_AT")
...
def _populate_unsealed_artifact_envelope(
    ...
    """Populate deterministic top-level fields after refs have been resolved."""

    findings["documentType"] = "code-security.findings"
    findings["schemaVersion"] = SCHEMA_VERSION
    coverage["documentType"] = "code-security.coverage"
...
def _normalize_unsealed_deep_repository_inventory_strategy(
    ...
    """Normalize the legacy deep-workflow label to the ordinary repository inventory."""
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    if manifest.get("documentType") != "code-security.scan-manifest":
        raise ContractError("manifest.documentType: expected code-security.scan-manifest")
...
def _validate_findings(manifest: dict[str, Any], findings: dict[str, Any]) -> None:
    if findings.get("documentType") != "code-security.findings":
        raise ContractError("findings.documentType: expected code-security.findings")
...
def _sarif_result(
    ...
    partial_fingerprints = {
        "codeSecurity/v1": finding["fingerprints"]["primary"],
    }
...
def build_sarif(
    ...
    run: dict[str, Any] = {
        ...
        "properties": {
            "codeSecuritySchemaVersion": manifest["schemaVersion"],
            "codeSecurityTargetKind": target["kind"],
        },
    }
...
def _write_sarif_projection_if_possible(
    ...
    try:
        ...
        print(
            f"code-security: warning: automatic SARIF export failed: {error}. "
            "Rerun the finalizer after correcting the reported SARIF error.",
            file=sys.stderr,
        )
...
def _prepare_scan_finalization(
    ...
    if not was_sealed:
        _populate_unsealed_manifest_envelope(manifest, scan)
    _validate_contract_refs(scan)
    ...
    if not was_sealed:
        _populate_unsealed_artifact_envelope(manifest, findings, coverage)
        _normalize_unsealed_deep_repository_inventory_strategy(coverage)
