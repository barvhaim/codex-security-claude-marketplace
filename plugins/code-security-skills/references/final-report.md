...
# Final Report and Host-Native Review Annotations

Use this guidance when authoring canonical report semantics and returning the generated Code Security report and review directives.
...
Every scan mode uses the same final report pipeline. The model authors canonical JSON only; it must not author, repair, or treat an existing `report.md` as input. Author `scan-manifest.json` as an unsealed draft and omit `scan.sealedAt` and `scan.artifacts`; the deterministic finalizer owns sealing, artifact digests, derived finding identities, and the Markdown/SARIF projections. Missing report prose must be added to the structured canonical fields rather than recovered from a separately authored report.

Run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/finalize_scan_contract.py" --scan-dir <scan_dir> --schema-dir "${CLAUDE_PLUGIN_ROOT}/schemas" --source-root <repo_root>` after writing the completed canonical JSON. Do not mark the scan complete until this command succeeds and the generated Markdown report exists.
...
If the local finalizer fails, stop and surface the exact error. Do not retry finalization blindly, return a final report or no-findings result, satisfy a structured output schema, or emit benchmark JSON. Preserve the scan directory for diagnosis or a later continuation.
...
- Offer tracking only when the `track-findings` workflow can use an available destination, such as Linear, Jira, or GitHub Issues. Name the destination in the question.
...
## Host-Native Review Annotations

When the active coding-agent host provides a documented review-annotation or inline-comment capability, the final response may project each surviving finding into that native format. Treat this as an optional presentation layer, not part of the canonical artifact contract.

- Use only syntax and fields documented by the active host; never invent a directive format.
- Keep the annotation title, explanation, file, line range, severity, and confidence consistent with `findings.json` and `report.md`.
- Use the tightest evidence-backed line range and one annotation per reportable finding.
- Emit no annotations when there are no reportable findings or when the host has no supported annotation capability.
- Do not let an annotation failure invalidate otherwise complete canonical artifacts.
