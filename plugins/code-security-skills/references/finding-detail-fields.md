...
For every reportable finding in `findings.json`, preserve the validated reasoning and the exact source snippets that prove it. The deterministic report and SARIF projections consume these fields directly; they do not recover missing analysis from `report.md` or reread source files after the scan.
...
- Keep code out of prose. Put source snippets in `codeEvidence[].code`, then reference them from the section that explains why the snippet matters. The report projection consolidates those referenced snippets under **Root cause** so the violated invariant and its source proof stay together.
...
Keep background exposition, alternate exploit research, full PoC instructions, representative command output, and long source walkthroughs in the detailed write-up. Do not copy them into canonical fields merely to make the projected report longer. The canonical finding should stay self-contained enough to support triage while avoiding duplicated or speculative prose.

The projected **Evidence** section is an artifact index, not another source-proof section. When `writeup.reportPath` is present, list that verified scan-local report plus validated regular files below its sibling `poc/` directory. A compatible host may make those paths navigable through a documented capability. Do not place artifact paths in root-cause prose or add an unvalidated artifact list to the canonical finding merely for display.
