...
Keep the total unknown while building the file list. Once the inventory is complete, record the exact file count in the parent-owned progress note or ledger, review files in bounded batches, and update the completed count after each batch using whatever progress mechanism the active host documents.
...
```text
python3 "${CLAUDE_PLUGIN_ROOT}"/scripts/normalize_candidates.py --input <candidate-source> [<candidate-source> ...] --out <discovery_dir>/candidate_ledger.jsonl --repo-root <repo_root> --in-scope-files <discovery_dir>/in_scope_files.txt
```
