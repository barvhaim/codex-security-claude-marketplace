...
- For JSONL worklist phases, use no more workers than the active host can run reliably. Keep each assignment self-contained, retain any returned handle only for host-supported result collection or cancellation, validate each result, and refill capacity only after a slot is free. If delegation is rejected or unavailable, have the parent complete the unstarted work without narrowing coverage. Do not assume a named worker protocol or use undocumented batch-import shortcuts.
- After each delegated file-review, validation, or attack-path dispatch, record the phase plus planned, started, completed, and fallback counts in the parent-owned progress note or ledger. Do not require a magic stdout marker or expose worker ids, paths, prompts, or errors in user-facing progress.
...
- For diff-scoped scans, generate `rank_input.jsonl` deterministically from changed source-like files with `python3 "${CLAUDE_PLUGIN_ROOT}"/scripts/generate_rank_input.py make-diff-rank-input --repo <repo_root> --base <base> --mode revisions --head <head> --out <discovery_dir>/rank_input.jsonl` for PR, commit, and branch diffs, or `python3 "${CLAUDE_PLUGIN_ROOT}"/scripts/generate_rank_input.py make-diff-rank-input --repo <repo_root> --base <base> --mode local-patch --out <discovery_dir>/rank_input.jsonl` for a local patch, then copy every row into `deep_review_input.jsonl` with `python3 "${CLAUDE_PLUGIN_ROOT}"/scripts/generate_rank_input.py copy-deep-review-input --rank-input <discovery_dir>/rank_input.jsonl --out <discovery_dir>/deep_review_input.jsonl`.
