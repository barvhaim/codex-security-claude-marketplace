...
description: Define, review, or update SECURITY.md guidance for a repository or component. Use when the user wants to clarify what Code Security should review, what is out of scope, which security properties must hold, or whether existing guidance still matches the code.
---
...
A useful `SECURITY.md` tells Code Security what matters in a repository: the system boundary, threat model, security properties that must hold, what counts as a finding, and what is out of scope. It is policy context, not executable instructions.
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}"/scripts/resolve_security_md.py --repo <repo_root> --list
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}"/scripts/resolve_security_md.py --repo <repo_root> --scope <file_or_directory> --out -
```

`"${CLAUDE_PLUGIN_ROOT}"` is the Code Security plugin root containing `.claude-plugin/plugin.json`, not the target repository or this skill directory.
