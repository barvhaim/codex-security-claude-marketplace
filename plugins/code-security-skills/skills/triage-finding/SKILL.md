...
That schema describes completed Code Security scan output. It requires generated fields such as `scanId`, `findingId`, `occurrenceId`, fingerprints,
...
Use the schema only as an optional compatibility source when the user supplies an existing `code-security.findings` JSON artifact. In that case, extract the available fields into the triage normalization record and preserve the original IDs as source identifiers. The triage result contract is defined in `references/triage-result-contract.md`.
...
an explicitly attached GitHub repository in the active agent session, or the current local repository's GitHub remote.

If the user asks to pull from GitHub without typing an `owner/repo` or URL, first infer the GitHub repository from an explicit repository attachment supplied by the active host when that metadata is available. Prefer that attachment over a local path or local git remote. If no explicit session attachment is visible, fall back to the current repository's GitHub remote. Only ask for a repository URL or `owner/repo`
...
SARIF results, CVE/GHSA or advisory descriptions, scanner tickets, bug bounty report snippets, Jira/Linear issue URLs or searches, Code Security finding artifacts, or a freeform vulnerability claim. If useful, ask for the repository path or affected file/component at the same time.
...
- input id, scanner id, SARIF rule/result id, CVE/GHSA id, ticket id, or Code Security `findingId`/`occurrenceId` when present
- title or short claim
- source type: `sarif`, `cve`, `advisory`, `scanner_ticket`,
  `bug_bounty`, `code_security_finding`, `freeform`, or `unknown`
...
3. Normalize each supplied or imported finding into a triage item.
   ...
   - Do not invent scanner fields, generated Code Security ids, severity, or remediation just to satisfy another schema.
...
13. Return a concise Markdown summary that preserves one row per supplied finding.
14. Include the fenced `triage-finding/v0` JSON block when the user requests machine-readable output or when a downstream integration explicitly requires it.
...
- `fix-finding` workflow handoff when verdict is `confirmed`

The optional JSON result must include:
...
The default response is the concise Markdown summary. Emit the fenced JSON block only when the user asks to see or copy the raw result contract or when an explicitly selected integration requires machine-readable output. If the active host offers a documented structured-results UI, use it only as an optional projection and keep the Markdown and JSON semantics unchanged.

## Fix-Finding Handoff

For `confirmed` findings, include a concise prompt-ready handoff for the `fix-finding` workflow with:
...
- proof gaps that the `fix-finding` workflow should preserve or validate

Do not invoke the `fix-finding` workflow unless the user explicitly asks to continue into fixing.
