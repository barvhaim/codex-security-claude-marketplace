...
Use this reference when the `triage-finding` workflow is invoked with a GitHub repository instead of pasted finding data.
...
- an explicitly attached GitHub repository in the active agent session
...
1. Current agent session's explicitly attached GitHub repository, when visible in the project context.
2. Current local repository's GitHub remote.
3. Explicit follow-up asking for `owner/repo` or a GitHub repository URL.

Prefer an explicit session attachment over a local path or local git remote. In the source-selection prompt, name the inferred `owner/repo` so the user can see which repository will be queried.
