...
Use this contract for first-pass output from the `triage-finding` workflow. The skill performs static, inline triage of supplied findings. It does not use a queue or deep triage mode.

## Schema Fit

`../../../schemas/findings.schema.json` is not the normalization shape for triage inputs.

That schema is for completed Code Security scan artifacts. It requires scan metadata, generated finding ids, occurrence ids, fingerprints, severity,
...
If the user supplies a valid `code-security.findings` artifact, use it as an input source and map its fields into this triage contract. Preserve `findingId`, `occurrenceId`, and `ruleId` as source identifiers; do not require new triage inputs to conform to the completed-scan schema.
