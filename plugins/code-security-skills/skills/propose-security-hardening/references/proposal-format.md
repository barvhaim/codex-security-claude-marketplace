...
Use this format for a derived hardening analysis based on vulnerability disclosures, supplied findings, incident or assessment documents, source evidence, a completed Code Security scan, or a mixture of these. The analysis is a design product, not part of its source evidence and not proof that any finding has been remediated.
...
```json
{
  "documentType": "code-security.hardening-analysis",
  "schemaVersion": "1.0",
  ...
  "sourceScan": {
    ...
    "snapshotDigest": "code-security-snapshot/v1:sha256:...",
    "sourceDrift": "none"
  },
  "assessment": {
...
Write `hardening.md` with these headings in order. Use `Evidence Basis` for an ordinary or mixed collection; `Source Scan` remains acceptable for analysis derived solely from a Code Security scan.
