...
- Use a configured Atlassian MCP server, CLI, API adapter, or host tool integration that exposes the required identity, project, issue metadata, search, mutation, and exact-readback capabilities. Reuse needs read access but not write access. Create and update need both. Stop if the integration is unavailable, disconnected, cannot read the destination, or cannot perform and verify the approved mutation.
