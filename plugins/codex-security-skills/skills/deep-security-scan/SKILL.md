...
> Claude Code adaptation notice: this file was adapted from OpenAI's Codex Security `deep-security-scan` skill for Claude Code. The original is Apache-2.0 licensed. This port replaces the Codex Security MCP coordinator with Claude Code subagents or repeated parent-agent passes.
...
1. Follow the setup and safety rules from `/codex-security-skills:security-scan`.
2. Create a scan directory outside the target repository, preferably under `~/.claude/security-scans/`.
...
1. Run at least three independent discovery passes when subagent capacity permits. Use separate Claude Code subagents with read-only instructions and no repository edits. If subagents are unavailable, run three explicitly separated parent-agent passes and reset the candidate hypothesis list between passes.
...
- Do not call Codex Security MCP tools; they are not bundled with this Claude plugin.
