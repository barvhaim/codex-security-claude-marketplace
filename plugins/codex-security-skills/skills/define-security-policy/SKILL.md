...
> Claude Code adaptation notice: this skill was adapted from OpenAI's Codex Security `define-security-policy` skill under Apache-2.0. Host-specific placeholders and namespaces were changed for Claude Code; see `${CLAUDE_PLUGIN_ROOT}/ADAPTATION.md` for compatibility limits.
...
`"${CLAUDE_PLUGIN_ROOT}"` is the Codex Security plugin root containing `.codex-plugin/plugin.json`, not the target repository or this skill directory.
