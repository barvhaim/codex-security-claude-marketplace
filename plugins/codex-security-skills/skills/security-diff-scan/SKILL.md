...
> Claude Code adaptation notice: this file was adapted from OpenAI's Codex Security `security-diff-scan` skill for Claude Code. The original is Apache-2.0 licensed. This port replaces Codex desktop and MCP scan lifecycle operations with local Git and Claude Code tools.
...
5. Create a scan directory outside the target tree, preferably under `~/.claude/security-scans/<repository>/<UTC timestamp>/`.
...
- Do not call Codex Security MCP tools; they are not part of this Claude plugin.
